...
def process_nodes(data, output, year, gender, min_age, max_age, report, node_ids):
    print(f"channel: {report['Name']} year: {year} gender: {gender} age_bin: [{min_age}:{max_age}) node_ids: {node_ids}")
    for node_id in node_ids:
        ...
        if len(rows) == 0 or missing_age_bracket:
            raise Exception(f"Channel {report['Name']} data is misaligned for year: {year} gender: {GENDER_STR[gender]} age_bin: [{min_age},{max_age}) .\n\nCommon causes:\n- simulation is not long enough\n- age(s) missing from Report_HIV_ByAgeAndGender.csv configuration parmeters\n- ingest form has a non-integer-year reference datum (must be integers)")
