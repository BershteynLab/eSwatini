...
MAX_MEMORY_MB = 8000
# Normally 1, but if you have additional memory needs for your sims, this is automatically increased.
# Does NOT change the number of cores used to run the sims, only the number requested (for memory resource purposes).
...
#ingest_xlsm_filename = os.path.join('Data', 'calibration_ingest_form-Swaziland--UPDATED--2024-01-14.xlsm')
ingest_xlsm_filename = os.path.join('Data', 'calibration_ingest_form-Swaziland--UPDATED--2024-04.xlsm')
...
province_names.append(PopulationObs.AGGREGATED_PROVINCE)  # to allow for national/aggregated parameter mapping
...
cfg.set_param('Memory_Usage_Warning_Threshold_Working_Set_MB', int(0.9 * MAX_MEMORY_MB), allow_new_parameters=True)
...
config_builder.params['Num_Cores'] = NUM_REQUESTED_CORES
...
def map_sample_to_model_input(sample_dict, template_set_name, scenario_name, campaign_filename, random_run_number=True):
    ...
    if ('Male To Female Young' in sample) and ('Male To Female Old' in sample):
